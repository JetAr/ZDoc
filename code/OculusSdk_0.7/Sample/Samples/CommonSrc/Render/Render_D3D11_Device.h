...
public:
    ...
    {
        return TextureSet ? ((ovrD3D11Texture*)&TextureSet->Textures[TextureSet->CurrentIndex])->D3D11.pTexture : Tex;
    }
    Ptr<ID3D11ShaderResourceView> GetSv() const
    {
        return TextureSet ? TexSv[TextureSet->CurrentIndex] : TexSv[0];
    }
    Ptr<ID3D11RenderTargetView> GetRtv() const
    {
        return TextureSet ? TexRtv[TextureSet->CurrentIndex] : TexRtv[0];
    }
    Ptr<ID3D11DepthStencilView> GetDsv() const
    {
        return TextureSet ? TexDsv[TextureSet->CurrentIndex] : TexDsv[0];
    }
    ...
    virtual int GetFormat() const OVR_OVERRIDE
    {
        return Format;
    }
