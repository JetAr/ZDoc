...
public:
    ...
    {
        OVR_UNUSED6(format, width, height, data, mipcount, error);
        return NULL;
    }
    ...
    {
        return new ShaderSetMatrixTranspose;
    }
    virtual Shader* LoadBuiltinShader(ShaderStage stage, int shader) = 0;

    // Rendering
    virtual void Blt(Texture* texture)
    {
        OVR_UNUSED(texture);
    }
