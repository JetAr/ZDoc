...
static XMVECTOR ConvertToXM(ovrQuatf q)
{
    return(XMVectorSet(q.x, q.y, q.z, q.w));
}
static XMFLOAT4 ConvertToXMF(ovrQuatf q)
{
    return(XMFLOAT4(q.x, q.y, q.z, q.w));
}
static XMVECTOR ConvertToXM(ovrVector3f v)
{
    return(XMVectorSet(v.x, v.y, v.z, 0));
}
static XMFLOAT3 ConvertToXMF(ovrVector3f v)
{
