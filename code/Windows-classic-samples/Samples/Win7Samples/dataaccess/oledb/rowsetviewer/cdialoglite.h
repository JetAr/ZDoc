...
public:
    ...
    HWND	GetDlgItem(INT iResourceID)
    {
        return ::GetDlgItem(m_hWnd, iResourceID);
    }
    UINT	IsDlgButtonChecked(INT iResourceID)
    {
        return ::IsDlgButtonChecked(m_hWnd, iResourceID);
    }
    BOOL	CheckDlgButton(INT iResourceID, UINT uCheck)
    {
        ASSERT(uCheck == BST_CHECKED || uCheck == BST_UNCHECKED || BST_INDETERMINATE);
        return ::CheckDlgButton(m_hWnd, iResourceID, uCheck);
    }
    ...
    HWND	GetWnd()
    {
        /*ASSERT(m_hWnd);*/ return m_hWnd;
    }
    BOOL	GetWindowPlacement()
    {
        memset(&m_wndPlacement, 0, sizeof(WINDOWPLACEMENT));
        m_wndPlacement.length	= sizeof(WINDOWPLACEMENT);
        return ::GetWindowPlacement(m_hWnd, &m_wndPlacement);
    }
    BOOL	SetWindowPlacement()
    {
        m_wndPlacement.length	= sizeof(WINDOWPLACEMENT);
        return ::SetWindowPlacement(m_hWnd, &m_wndPlacement);
    }

    WCHAR*	GetWindowText()
    {
        return wGetWindowText(m_hWnd);
    }
    LRESULT	GetWindowText(WCHAR* pwszBuffer, ULONG ulMaxSize)
    {
        return wSendMessage(m_hWnd, WM_GETTEXT, ulMaxSize, pwszBuffer);
    }
    BOOL	SetWindowText(WCHAR* pwszString)
    {
        return (BOOL)wSendMessage(m_hWnd, WM_SETTEXT, 0,		 pwszString);
    }

    LONG	ShowWindow(UINT nCmdShow)
    {
...
//protected:
    ...
    {
        return !m_hWnd && !m_hWndParent && !m_pSubClassProc && !m_pwszClassName;
    }
...
public:
    ...
    //Helpers
    virtual void	MDIActivate(HWND hWnd)
    {
        SendMessage(m_hWndMDIClient, WM_MDIACTIVATE, (WPARAM)hWnd, 0);
    }
    virtual void	MDIDestroy(HWND hWnd)
    {
    ...
    virtual	BOOL	IsDestroyed()
    {
        return !m_hWndMDIClient && CFrameWndLite::IsDestroyed();
    }
...
public:
    ...
    //Interface
    virtual void	SetSel(INDEX iMin, INDEX iMax)
    {
        SendMessage(m_hWnd, EM_SETSEL, (WPARAM)iMin, (LPARAM)iMax);
    }
    virtual BOOL	GetSel(INDEX* piMin, INDEX* piMax)
    {
        CHARRANGE cr;
        BOOL bReturn = GetSel(&cr);
        if(piMin) *piMin = cr.cpMin;
        if(piMax) *piMax = cr.cpMax;
        return bReturn;
    }
    virtual BOOL	GetSel(CHARRANGE* pcr)
    {
        ASSERT(pcr);
        SendMessageA(m_hWnd, EM_GETSEL, (WPARAM)&pcr->cpMin, (LPARAM)&pcr->cpMax);
        return pcr->cpMin != pcr->cpMax;
    }
    ...
    INDEX			LineIndex(INDEX nLine)
    {
        return (INDEX)SendMessage(m_hWnd, EM_LINEINDEX, nLine, 0);
    }
    INDEX			LineLength(INDEX nStartChar)
    {
        return (INDEX)SendMessage(m_hWnd, EM_LINELENGTH, nStartChar, 0);
    }
    void			LineScroll(INDEX nLines, INDEX nChars = 0)
    {
        SendMessage(m_hWnd, EM_LINESCROLL, nChars, nLines);
    }
    LRESULT			SetOptions(UINT dwOptions, UINT fOperation = ECOOP_OR)
    {
        return SendMessage(m_hWnd, EM_SETOPTIONS, fOperation, dwOptions);
    }

    BOOL			CanPaste()
    {
        return (BOOL)SendMessage(m_hWnd, EM_CANPASTE, 0, 0);
    }
    void			Cut()
    {
    ...
    BOOL			ScrollCaret()
    {
        return (BOOL)SendMessage(m_hWnd, EM_SCROLLCARET, 0, 0);
    }

    virtual void	ReplaceSel(LPCWSTR pwszString, bool bCanUndo)
    {
        wSendMessage(m_hWnd, EM_REPLACESEL, (WPARAM)bCanUndo, (WCHAR*)pwszString);
    }
    virtual void	ReplaceAll(LPCWSTR pwszString, bool bReplaceAll = FALSE, bool fHighlight = FALSE);

    INDEX			GetLineCount()
    {
        return (INDEX)SendMessage(m_hWnd, EM_GETLINECOUNT, 0, 0);
    }
...
public:
    ...
    virtual void	SetSel(INDEX iMin, INDEX iMax)
    {
        CHARRANGE cr = { (LONG)iMin, (LONG)iMax };
    ...
    {
        ASSERT(pcr);
        SendMessageA(m_hWnd, EM_EXGETSEL, 0, (LPARAM)pcr);
        return pcr->cpMin != pcr->cpMax;
    }

    //Inline helpers, (that deal with hWnds)
    INDEX			LineFromChar(INDEX nIndex)
    {
        return (INDEX)SendMessage(m_hWnd, EM_EXLINEFROMCHAR, 0, nIndex);
    }
...
public:
    ...
    virtual BOOL	SetScrollRange( int nMin, int nMax, BOOL bRedraw = TRUE )
    {
        return ::SetScrollRange(m_hWnd, SB_CTL, nMin, nMax, bRedraw);
    }
    virtual BOOL	GetScrollRange( int* pnMin, int* pnMax )
    {
        return ::GetScrollRange(m_hWnd, SB_CTL, pnMin, pnMax);
    }
...
public:
    ...
    //Inline helpers, (that deal with hWnds)
    virtual INDEX	SetCurSel(WPARAM nSelect)
    {
        return (INDEX)::SendMessage(m_hWnd,	LB_SETCURSEL, nSelect, 0);
    }
    virtual INDEX	GetCurSel() const
    {
        return (INDEX)::SendMessage(m_hWnd,	LB_GETCURSEL, 0, 0);
    }
    virtual INDEX	AddString(WCHAR* pwszString)
    {
        return (INDEX)wSendMessage(m_hWnd,	LB_ADDSTRING,	0, pwszString);
    }
    virtual INDEX	DeleteString(WPARAM wParam)
    {
        return (INDEX)wSendMessage(m_hWnd,	LB_DELETESTRING, wParam, 0);
    }
    virtual INDEX	GetText(INDEX nIndex, WCHAR* pwszBuffer)
    {
        return (INDEX)wSendMessage(m_hWnd,	LB_GETTEXT, nIndex, pwszBuffer);
    }
    virtual INDEX	GetTextLen(INDEX nIndex)
    {
        return (INDEX)::SendMessage(m_hWnd,	LB_GETTEXTLEN, nIndex, 0);
    }

    virtual BOOL	SetItemParam(INDEX nIndex, LPARAM lParam)
    {
        return ::SendMessage(m_hWnd,	LB_SETITEMDATA, nIndex, lParam) != CB_ERR;
    }
    virtual LPARAM	GetItemParam(INDEX nIndex) const
    {
        return ::SendMessage(m_hWnd,	LB_GETITEMDATA, nIndex, 0);
    }

    virtual INDEX	GetCount()
    {
        return (INDEX)::SendMessage(m_hWnd, LB_GETCOUNT, 0, 0);
    }
    //Helpers
...
public:
    ...
    //Inline helpers, (that deal with hWnds)
    BOOL			DeleteColumn(INDEX nCol)
    {
        return (BOOL)SendMessage(m_hWnd, LVM_DELETECOLUMN, nCol, 0);
    }
    BOOL			DeleteItem(INDEX iItem)
    {
        return (BOOL)SendMessage(m_hWnd, LVM_DELETEITEM, iItem, 0L);
    }
    BOOL			DeleteAllItems()
    {
        return (BOOL)SendMessage(m_hWnd, LVM_DELETEALLITEMS, 0, 0L);
    }
    INDEX			GetNextItem(INDEX iItem, INT nFlags)
    {
        return (INDEX)SendMessage(m_hWnd, LVM_GETNEXTITEM, iItem, MAKELPARAM(nFlags, 0));
    }
    INDEX			GetColumnWidth(INDEX nCol)
    {
        return (INDEX)SendMessage(m_hWnd, LVM_GETCOLUMNWIDTH, nCol, 0);
    }
    BOOL			SetColumnWidth(INDEX nCol, INT cx)
    {
        return (BOOL)SendMessage(m_hWnd, LVM_SETCOLUMNWIDTH, nCol, MAKELPARAM(cx, 0));
    }
    UINT			GetItemState(INDEX iItem, UINT nMask)
    {
        return (UINT)SendMessage(m_hWnd, LVM_GETITEMSTATE, iItem, nMask);
    }
    BOOL			EnsureVisible(INDEX iItem, bool fPartialOK = FALSE)
    {
        return (BOOL)SendMessage(m_hWnd, LVM_ENSUREVISIBLE, iItem, MAKELPARAM(fPartialOK, 0));
    }
    INDEX			GetItemCount()
    {
        return (INDEX)SendMessage(m_hWnd, LVM_GETITEMCOUNT, 0, 0);
    }
    INDEX			GetCountPerPage()
    {
        return (INDEX)SendMessage(m_hWnd, LVM_GETCOUNTPERPAGE, 0, 0);
    }
    HWND			EditLabel(INDEX iItem)
    {
        return (HWND)SendMessage(m_hWnd, LVM_EDITLABEL, iItem, 0L);
    }
...
public:
    ...
    BOOL			Expand(HTREEITEM hItem, UINT flag = TVE_EXPAND)
    {
        ASSERT(IsWindow(m_hWnd));
        return (BOOL)SendMessage(m_hWnd, TVM_EXPAND, flag, (LPARAM)hItem);
    }
...
public:
    ...
    virtual INDEX		GetCount()
    {
        return (INDEX)::SendMessage(m_hWnd,	CB_GETCOUNT, 0, 0);
    }
    virtual INDEX		GetCurSel() const
    {
        return (INDEX)::SendMessage(m_hWnd,	CB_GETCURSEL, 0, 0);
    }
    virtual INDEX		SetCurSel(WPARAM nSelect)
    {
        return (INDEX)::SendMessage(m_hWnd,	CB_SETCURSEL, nSelect, 0);
    }

    virtual INDEX		AddString(WCHAR* pwszString)
    {
        return (INDEX)::wSendMessage(m_hWnd,	CB_ADDSTRING,	0, pwszString);
    }
    virtual INDEX		DeleteString(INDEX nIndex)
    {
        return (INDEX)::SendMessage(m_hWnd,	CB_DELETESTRING, nIndex, 0);
    }
    virtual BOOL		SetItemParam(INDEX nIndex, LPARAM lParam)
    {
        return ::SendMessage(m_hWnd,	CB_SETITEMDATA, nIndex, lParam) != CB_ERR;
    }
    ...
    virtual LRESULT		GetTextLength(INDEX nIndex)
    {
        return ::SendMessage(m_hWnd,	CB_GETLBTEXTLEN, nIndex, 0);
    }
    virtual INDEX		GetText(INDEX nIndex, WCHAR* pwszBuffer)
    {
        return (INDEX)::wSendMessage(m_hWnd,	CB_GETLBTEXT, nIndex, pwszBuffer);
    }
    virtual INDEX		FindString(WCHAR* pwszString, INDEX nStartIndex = -1)
    {
        return (INDEX)wSendMessage(m_hWnd,	CB_FINDSTRING,		nStartIndex, pwszString);
    }
    virtual INDEX		FindStringExact(WCHAR* pwszString, INDEX nStartIndex = -1)
    {
        return (INDEX)wSendMessage(m_hWnd,	CB_FINDSTRINGEXACT,	nStartIndex, pwszString);
    }
    ...
    //Helpers
    virtual INDEX		SaveSelection()
    {
        return m_iSavedSel = GetCurSel();
    }
    virtual	INDEX		RestoreSelection()
    {
        return SetCurSel(m_iSavedSel);
    }
...
public:
    ...
    //Inline helpers, (that deal with hWnds)
    virtual UINT	IsButtonChecked()
    {
        return ::IsDlgButtonChecked(m_hWndParent, m_nID);
    }
    virtual BOOL	CheckButton(UINT uCheck)
    {
        ASSERT(uCheck == BST_CHECKED || uCheck == BST_UNCHECKED || BST_INDETERMINATE);
        return ::CheckDlgButton(m_hWndParent, m_nID, uCheck);
    }
...
public:
    ...
    virtual INDEX		AddString(WCHAR* pwszString)
    {
        return InsertItem(-1/*iItem*/, pwszString);
    }
    virtual INDEX		AddString(WCHAR* pwszString, LPARAM lParam)
    {
        return InsertItem(-1/*iItem*/, pwszString, lParam);
    }

    virtual BOOL		SetUnicodeFormat(BOOL fUnicode = TRUE)
    {
        return (BOOL)::SendMessage(m_hWnd, CBEM_SETUNICODEFORMAT, fUnicode, 0);
    }

    //NOTE: FindString is not processed by ComboBoxEx, but FindStringExact is...
    virtual INDEX		FindString(WCHAR* pwszString, INDEX nStartIndex = -1)
    {
        ASSERT(!L"FindString is not processed by ComboBoxEx, change code to use FindStringExact.");
        return FindStringExact(pwszString, nStartIndex);
    }
...
public:
    ...
    //Helpers
    virtual BOOL EnableButton(UINT nID, bool fEnable)
    {
        return (BOOL)SendMessage(m_hWnd, TB_ENABLEBUTTON, nID, fEnable);
    }
};
...
public:
    ...
    //Helpers
    virtual BOOL SetText(WCHAR* pwszText, int nPane, int nType)
    {
        return (BOOL)wSendMessage(m_hWnd, SB_SETTEXTW, (nPane|nType), pwszText);
    }
    virtual BOOL SetParts(int nParts, int* pWidths)
    {
...
public:
    ...
    virtual CPropSheetLite*		GetParent()
    {
        return m_pCPropSheet;
    }
...
public:
    ...
    //Inlines
    virtual INDEX	GetItemCount()
    {
        return (INDEX)::SendMessageA(m_hWnd, TCM_GETITEMCOUNT, 0, 0);
    }
    virtual INDEX	GetCurSel() const
    {
        return (INDEX)::SendMessageA(m_hWnd, TCM_GETCURSEL, 0, 0);
    }
    virtual INDEX	SetCurSel(INDEX iItem, BOOL fSendNotification = FALSE);

    virtual BOOL	DeleteAllItems()
    {
        return (BOOL)::SendMessageA(m_hWnd, TCM_DELETEALLITEMS, 0, 0);
    }
    virtual BOOL	DeleteItem(INDEX iItem)
    {
        return (BOOL)::SendMessageA(m_hWnd, TCM_DELETEITEM, iItem, 0);
    }
    virtual void	AdjustRect(RECT* pRect, BOOL fLarger = FALSE)
    {
        SendMessageA(m_hWnd, TCM_ADJUSTRECT, fLarger, (LPARAM)pRect);
    }
