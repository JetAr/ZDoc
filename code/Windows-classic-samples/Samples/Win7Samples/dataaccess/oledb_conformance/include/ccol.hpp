...
public:
    ...
    inline WCHAR*	GetPrefix()
    {
        return m_pwszPrefix;
    }

    // @cmember,mfunc Sets column Ordinal.
    inline void SetColNum(DBORDINAL iOrdinal = 0)
    {
        m_ColInfo.iOrdinal = iOrdinal;
    }
    inline DBORDINAL GetColNum()
    {
        return m_ColInfo.iOrdinal;
    }

    // @cmember,mfunc Sets provider type.
    inline void		SetProviderType(DBTYPE wType)
    {
        m_ColInfo.wType = wType;
    }
    inline DBTYPE	GetProviderType()
    {
        return m_ColInfo.wType;
    }

    // @cmember,mfunc Sets SubType, m_wSubType.
    inline void		SetSubType(DBTYPE wSubType)
    {
        m_wSubType = wSubType;
    }
    inline DBTYPE	GetSubType()
    {
        return m_wSubType;
    }
    ...
    inline void		SetColumnSize(DBLENGTH ulColumnSize)
    {
        /*ASSERT(!IsFixedLength(GetProviderType()));*/ m_ColInfo.ulColumnSize = ulColumnSize;
    }
    inline DBLENGTH	GetColumnSize()
    {
        /*ASSERT(!IsFixedLength(GetProviderType()));*/ return m_ColInfo.ulColumnSize;
    }
    DBLENGTH		GetMaxColumnSize(void);

    // @cmember,mfunc Set data source's precision for this datatype, bPrecision.
    inline void		SetPrecision(BYTE bPrecision)
    {
        /*ASSERT(IsNumericType(GetProviderType()) || GetProviderType()==DBTYPE_DBTIMESTAMP);*/ m_ColInfo.bPrecision = bPrecision;
    }
    inline BYTE		GetPrecision()
    {
        /*ASSERT(IsNumericType(GetProviderType()) || GetProviderType()==DBTYPE_DBTIMESTAMP);*/ return m_ColInfo.bPrecision;
    }

    // @cmember,mfunc Set data source's scale for this data type, bScale.
    inline void		SetScale(BYTE bScale)
    {
        /*ASSERT(IsScaleType(GetProviderType()));*/ m_ColInfo.bScale = bScale;
    }
    inline BYTE		GetScale()
    {
        /*ASSERT(IsScaleType(GetProviderType()));*/ return m_ColInfo.bScale;
    }

    // @cmember,mfunc Set data source's scale for this data type, m_sMinScale.
    inline void		SetMinScale(SHORT sScale)
    {
        /*ASSERT(IsScaleType(GetProviderType()));*/ m_sMinScale=sScale;
    }
    inline SHORT	GetMinScale()
    {
        /*ASSERT(IsScaleType(GetProviderType()));*/ return m_sMinScale;
    }

    // @cmember,mfunc Set data source's scale for this data type, m_sMaxScale.
    inline void		SetMaxScale(SHORT sScale)
    {
        /*ASSERT(IsScaleType(GetProviderType()));*/ m_sMaxScale=sScale;
    }
    inline SHORT	GetMaxScale()
    {
        /*ASSERT(IsScaleType(GetProviderType()));*/ return m_sMaxScale;
    }

    // @cmember,mfunc Set data source's ability to null this data type,
    inline void		SetNullable(BOOL fNullable=0)
    {
    ...
    inline void		SetUnsigned(BOOL fUnsigned=FALSE)
    {
        m_fUnsigned=fUnsigned;
    }
    inline BOOL		GetUnsigned()
    {
        return m_fUnsigned;
    }

    inline void			SetTypeInfo(ITypeInfo* pTypeInfo)
    {
        m_ColInfo.pTypeInfo = pTypeInfo;
        SAFE_ADDREF(pTypeInfo);
    }
    inline ITypeInfo*	GetTypeInfo()
    {
        return m_ColInfo.pTypeInfo;
    }
    ...
    inline void		SetAutoInc(BOOL fAutoInc=FALSE)
    {
        m_fAutoInc=fAutoInc;
    }
    inline BOOL		GetAutoInc()
    {
        return m_fAutoInc;
    }

    // @cmember,mfunc Set data type's ability to support autoinc columns
    inline void		SetCanAutoInc(BOOL fAutoInc=FALSE)
    {
        m_fCanAutoInc=fAutoInc;
    }
    inline BOOL		CanAutoInc()
    {
        return m_fCanAutoInc;
    }

    // @cmember,mfunc Set column values are unique
    inline BOOL		SetUnique(BOOL fUnique=FALSE)
    {
        return m_fUnique=fUnique;
    }
    ...
    inline void		SetUpdateable(BOOL fUpdateable=FALSE)
    {
        m_fUpdateable=fUpdateable;
    }
    inline BOOL		GetUpdateable()
    {
        return m_fUpdateable;
    }

    // @cmember,mfunc Set data source's ability to search this data type,
    inline void		SetSearchable(ULONG ulSearchable=0)
    {
        m_ulSearchable=ulSearchable;
    }
    inline ULONG	GetSearchable()
    {
        return m_ulSearchable;
    }

    // @cmember,mfunc Return GetColUsed, m_fUseInSQL.
    inline void		SetUseInSQL(BOOL fColUsed=FALSE)
    {
        m_fUseInSQL=fColUsed;
    }
    inline BOOL		GetUseInSQL()
    {
        return m_fUseInSQL;
    }

    // @cmember,mfunc Set data source's ability to contain long data,
    inline void		SetIsLong(BOOL fIsLong=FALSE)
    {
        m_fIsLong=fIsLong;
    }
    inline BOOL		GetIsLong()
    {
        return m_fIsLong;
    }

    // @cmember,mfunc SetIsChapter
    inline void		SetIsChapter(BOOL fIsChapter)
    {
        ENABLE_BIT(m_ColInfo.dwFlags, DBCOLUMNFLAGS_ISCHAPTER, fIsChapter);
    }

    // @cmember, mfunc Return whether column is fixed length
    inline void		SetIsFixedLength(BOOL fIsFixedLength=FALSE)
    {
        m_fIsFixedLength=fIsFixedLength;
    }
    inline BOOL		GetIsFixedLength()
    {
        return m_fIsFixedLength;
    }

    // @cmember, mfunc Sets whether column has fixed len data or not
    inline void		SetCaseSensitive(BOOL fCaseSensitive=FALSE)
    {
        m_fCaseSensitive=fCaseSensitive;
    }
    inline BOOL		GetCaseSensitive()
    {
        return m_fCaseSensitive;
    }

    // @cmember,mfunc Set Column'sType Guid
    inline void		SetTypeGuid(GUID gTypeGuid)
    {
        memcpy(&m_gTypeGuid,&gTypeGuid,sizeof(GUID));
    }
    inline GUID		GetTypeGuid()
    {
        return m_gTypeGuid;
    }

    // @cmember,mfunc Set Column's DefaultValue
    inline HRESULT	SetDefaultValue(VARIANT DefaultValue)
    {
        return VariantCopy(&m_DefaultValue, &DefaultValue);
    }
    inline VARIANT	GetDefaultValue()
    {
        return m_DefaultValue;
    }

    // @cmember,mfunc Set whether the Column has DefaultValue
    inline BOOL		SetHasDefault(BOOL HasDefault)
    {
        return m_fHasDefault=HasDefault;
    }
    inline BOOL		GetHasDefault()
    {
        return m_fHasDefault;
    }

    // @cmember,mfunc Set whether the user has explicitly requested creation parameters
    inline void		SetReqParams(ULONG cReqParams=MAX_PTR)
    {
        m_cReqParams=cReqParams;
    }
    inline ULONG	GetReqParams()
    {
        return m_cReqParams;
    }
    ...
    inline ULONG	GetLCID()
    {
        return m_ulLCID;
    }

    // @cmember,mfunc Set when this column is a new LOB column
    void			SetIsNewLongType(BOOL fIsNewLongType)
    {
        m_fIsNewLongType = fIsNewLongType;
    }
    BOOL			GetIsNewLongType()
    {
        return m_fIsNewLongType;
    }
