...
public:
    ...
    virtual inline ICommand * get_ICommandPTR(void)
    {
        return m_pICommand;
    };
    virtual inline void set_ICommandPTR( ICommand *pICommand)
    {
    ...
    void AddRow(DBCOUNTITEM ulRow=1)
    {
        m_ulRows += ulRow;
        m_ulNextRow += ulRow;
    };
    ...
    void SubtractRow(DBCOUNTITEM ulRow=1)
    {
        if(m_ulRows>0) m_ulRows -= ulRow;
        if(m_ulNextRow>0) m_ulNextRow -= ulRow;
    };
    ...
    DBCOUNTITEM GetRowsOnCTable(void)
    {
        return m_ulRows;
    };
    ...
    WCHAR * GetTableName(void)
    {
        return m_TableID.uName.pwszName;
    };

    // @cmember,mfunc Returns ViewName, client should not alter pointer.
    WCHAR * GetViewName(void)
    {
        return m_pwszViewName;
    };

    // @cmember,mfunc Returns TableID
    DBID GetTableID(void)
    {
        return m_TableID;
    };

    // @cmember,mfunc Returns the reference of TableID
    DBID& GetTableIDRef(void)
    {
        return m_TableID;
    };
    ...
    WCHAR * GetIndexName(void)
    {
        return m_pwszIndexName;
    }
    ...
    LONG_PTR GetSQLSupport(void)
    {
        return m_lSQLSupport;
    }

    // @cmember Flag telling if the Provider is ReadOnly.
    BOOL GetIsProviderReadOnly(void)
    {
        return m_fProviderReadOnly;
    }
