...
public:
    ...
    inline DBORDINAL GetPrimaryKeyColumn(void)
    {
        return m_ulPrimaryKey;
    }

    // Sets Column number which Primary key is created on.
    // Note!!!: if m_eCreatePK==CREATENEVER_PK then m_ulPrimaryKey will be ignored
    inline void SetPrimaryKeyColumn(DBORDINAL ulPrimaryKey)
    {
        m_ulPrimaryKey=ulPrimaryKey;
    }

    // Returns m_eCreatePK
    inline ECREATE_PK GetCreatePK(void)
    {
        return m_eCreatePK;
    }
