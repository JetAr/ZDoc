...
//#endif//TXNJOIN
    inline void *GetVarInfo(const long)
    {
        return (void *)NULL;
    }
    //@cmember Dummy necessary virtual function needed to
    //instantiate since we inherit from CTestCases
    inline const WCHAR *GetCaseDesc(void)
    {
        return (const WCHAR *)NULL;
    }
...
public:
    ...
    inline ULONG	GetNextRowToInsert()
    {
        return g_ulLastActualInsert + 1;
    };
    //@cmember  Gets value to use for next delete
    inline ULONG	GetNextRowToDelete()
    {
        return g_ulLastActualDelete + 1;
    };
