...
CLEANUP:
    ...
    if(hAccessor)
    {
        ...
        else if(CHECK(pIRowsetChange->QueryInterface(IID_IAccessor,(LPVOID *)&pIAccessor),S_OK))
        {
            CHECK(pIAccessor->ReleaseAccessor(hAccessor,NULL),S_OK);
            SAFE_RELEASE(pIAccessor);
        }
...
CLEANUP:
    ...
    if(hAccessor)
    {
        ...
        else if(CHECK(pIRowsetChange->QueryInterface(IID_IAccessor,
                      (LPVOID *)&pIAccessor),S_OK))
        {
            CHECK(pIAccessor->ReleaseAccessor(hAccessor,NULL),S_OK);
            SAFE_RELEASE(pIAccessor);
...
BOOL InsertDefault::GetDefaultColumns()
{
    ...
    while (Rowset.GetNextRows(&hRow) == S_OK)
    {
        ...
        else
        {
            ...
            else if (DBSTATUS_S_OK == STATUS_BINDING(*pColGUIDBinding, Rowset.m_pData))
            {
        ...
        if (VARIANT_TRUE == *(VARIANT_BOOL*)((BYTE*)Rowset.m_pData+pHasDefaultBinding->obValue))
        {
            ...
            else if (DBSTATUS_S_ISNULL == STATUS_BINDING(*pDefaultBinding, Rowset.m_pData))
            {
