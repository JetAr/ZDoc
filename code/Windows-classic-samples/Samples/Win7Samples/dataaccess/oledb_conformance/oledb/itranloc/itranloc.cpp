...
int TCITransactionObject::Variation_1()
{
    ...
    if (CHECK((rgpITransaction[8])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK))
    {
        goto CLEANUP;
    }
    //since 8 was committed 9 is implicity committed as well
    if (CHECK((rgpITransaction[9])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), XACT_E_NOTRANSACTION))
    {
        goto CLEANUP;
    }

    if (CHECK((rgpITransaction[7])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK))
    {
        goto CLEANUP;
    }
    if (CHECK((rgpITransaction[6])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK))
    {
        goto CLEANUP;
    }
    if (CHECK((rgpITransaction[5])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK))
    {
        goto CLEANUP;
    }
    if (CHECK((rgpITransaction[4])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK))
    {
        goto CLEANUP;
    }
    if (CHECK((rgpITransaction[3])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK))
    {
        goto CLEANUP;
    }
    if (CHECK((rgpITransaction[0])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK))
    {
        goto CLEANUP;
    }
    //since 0 was committed 1 and 2 are implicity committed as well
    if (CHECK((rgpITransaction[2])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK))
    {
        goto CLEANUP;
    }
    if (CHECK((rgpITransaction[1])->Commit(FALSE, XACTTC_SYNC_PHASETWO, 0), S_OK))
    {
        goto CLEANUP;
    }
