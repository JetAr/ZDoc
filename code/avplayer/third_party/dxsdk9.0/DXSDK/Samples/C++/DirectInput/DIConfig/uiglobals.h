...
public:
    ...
    LPDICONFIGUREDEVICESCALLBACK GetCallback()
    {
        return m_lpCallback;
    }
    LPVOID GetRefData()
    ...
    {
        return AreFlagsSet(dwFlag);
    }
    BOOL AreFlagsSet(DWORD dwFlags)
    {
        return (m_dwFlags & dwFlags) == dwFlags;
    }
    BOOL InEditMode()
    {
        return IsFlagSet(DICD_EDIT);
    }
    const DIACTIONFORMATW &RefMasterAcFor(int i);
    int GetNumMasterAcFors()
    {
        return m_MasterAcForArray.GetSize();
    }
    const DICOLORSET &GetColorSet() const
    {
        return m_ColorSet;
    }
...
public:
    ...
    BOOL LineTo(SPOINT p)
    {
        return LineTo(p.x, p.y);
    }
    BOOL LineTo(int x, int y);
    BOOL MoveTo(SPOINT p, SPOINT *last = NULL)
    {
        return MoveTo(p.x, p.y, last);
    }
