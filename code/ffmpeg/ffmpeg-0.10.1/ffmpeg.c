...
//                fprintf(stderr, "drift:%f len:%d opts:%"PRId64" ipts:%"PRId64" fifo:%d\n", delta, -1, ost->sync_opts, (int64_t)(get_sync_ipts(ost) * enc->sample_rate), av_fifo_size(ost->fifo)/(ost->st->codec->channels * 2));
    ...
    {
        buftmp = audio_buf;
        size_out = swr_convert(ost->swr, (      uint8_t*[])
        {
            buftmp
        }, audio_buf_size / (enc->channels * osize),
        (const uint8_t*[])
        {
            buf
    ...
    *pkt_pts  = AV_NOPTS_VALUE;
    ...
    for(i=0; i<nb_output_streams; i++)
    {
        ...
        if(check_output_constraints(ist, ost) && ost->encoding_needed)
        {
            ...
            else if((av_vsrc_buffer_add_frame(ost->input_video_filter, decoded_frame, AV_VSRC_BUF_FLAG_OVERWRITE)) < 0)
            {
        ...
        else
        {
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (!ost->frame_rate.num)
                    ost->frame_rate = ist->st->r_frame_rate.num ? ist->st->r_frame_rate : (AVRational)
                {
                    25, 1
                };
                ...
                codec->time_base = (AVRational)
                {
                    ost->frame_rate.den, ost->frame_rate.num
                };
    ...
    else
    {
        for (i = 0; i < o->nb_stream_maps; i++)
        {
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                break;
            case AVMEDIA_TYPE_AUDIO:
                ost = new_audio_stream(o, oc);
                break;
            case AVMEDIA_TYPE_SUBTITLE:
                ost = new_subtitle_stream(o, oc);
                break;
            case AVMEDIA_TYPE_DATA:
                ost = new_data_stream(o, oc);
                break;
            case AVMEDIA_TYPE_ATTACHMENT:
