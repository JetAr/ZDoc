...
static int query_formats(AVFilterContext *ctx)
{
    ...
    if(out_rate > 0)
    {
        out_samplerates = avfilter_make_format_list((int[])
        {
            out_rate, -1
        });
    }
    ...
    {
        out_formats = avfilter_make_format_list((int[])
        {
            out_format, -1
        });
    }
    ...
    if(out_layout)
    {
        out_layouts = avfilter_make_format64_list((int64_t[])
        {
            out_layout, -1
        });
    }
...
static int request_frame(AVFilterLink *outlink)
{
    ...
    if (ret == AVERROR_EOF)
    {
        ...
        if(aresample->next_pts != AV_NOPTS_VALUE)
            aresample->next_pts += av_rescale_q(n_out, (AVRational)
        {
