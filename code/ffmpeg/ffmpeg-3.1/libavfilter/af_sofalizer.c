...
static int load_sofa(AVFilterContext *ctx, char *filename, int *samplingrate)
{
    ...
    *samplingrate = sample_rate; /* remember sampling rate */
    ...
    status += nc_get_vara_float(ncid, sp_id, (size_t[2])
    {
        0, 0
    } ,
    (size_t[2])
    {
        m_dim, 1
    }, sp_a); /* read & store azimuth angles */
    status += nc_get_vara_float(ncid, sp_id, (size_t[2])
    {
        0, 1
    } ,
    ...
    {
        m_dim, 1
    }, sp_e); /* read & store elevation angles */
    status += nc_get_vara_float(ncid, sp_id, (size_t[2])
    {
        0, 2
    } ,
...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    td.ringbuffer = s->ringbuffer;
    td.temp_src = s->temp_src;
