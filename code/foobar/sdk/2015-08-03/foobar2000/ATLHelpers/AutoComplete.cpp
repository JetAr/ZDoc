...
public:
    ...
    CEnumString(const t_data & in) : m_data(in)
    {
        m_walk = m_data.first();
    }
    CEnumString() {}
...
{
public:
    CACList_History(cfg_dropdown_history_mt * var) : m_var(var)
    {
        Reset();
    }
...
HRESULT InitializeSimpleAC(HWND edit, IUnknown * vals, DWORD opts)
{
    ...
    if (FAILED(hr))
    {
        PFC_ASSERT(!"Should not get here - CoCreateInstance/IAutoComplete fail!");
        return hr;
    }
    hr = ac->Init(edit, vals, NULL, NULL);
    if (FAILED(hr))
    {
        PFC_ASSERT(!"Should not get here - ac->Init fail!");
        return hr;
    }
    ...
    if (FAILED(hr))
    {
        PFC_ASSERT(!"Should not get here - ac->QueryInterface fail!");
        return hr;
    }
    hr = ac2->SetOptions(opts);
    if (FAILED(hr))
    {
        PFC_ASSERT(!"Should not get here - ac2->SetOptions fail!");
        return hr;
    }
    return S_OK;
}
