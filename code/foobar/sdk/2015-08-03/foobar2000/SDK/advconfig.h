...
public:
    ...
    {
        return 0;   //signals whether changing this setting should trigger playback restart or app restart; see: preferences_state::* constants
    }
};
...
public:
    ...
    bool get() const
    {
        return this->get_static_instance().get_state_();
    }
    void set(bool val)
    {
        this->get_static_instance().set_state(val);
    }
    operator bool() const
    {
        return get();
...
public:
    ...
    {
        get_static_instance().get_state(out);
    }
    void set(const char * in)
    {
        get_static_instance().set_state(in);
    }
};
...
public:
    ...
    void get_state(pfc::string_base & p_out)
    {
        p_out = pfc::format_uint(m_state.get_value());
    }
    void set_state(const char * p_string,t_size p_length)
    {
        set_state_int(pfc::atoui64_ex(p_string,p_length));
    }
    t_uint32 get_flags()
    {
        return advconfig_entry_string::flag_is_integer;
    }
    ...
    {
        m_state = pfc::clip_t<t_uint64>(val,m_min,m_max);
    }
...
public:
    ...
    t_uint64 get() const
    {
        return get_static_instance().get_state_int();
    }
    void set(t_uint64 val)
    {
        get_static_instance().set_state_int(val);
    }
