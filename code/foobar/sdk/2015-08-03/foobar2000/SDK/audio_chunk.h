...
public:
    ...
    {
        return p_val >= sample_rate_min && p_val <= sample_rate_max;
    }
    ...
    inline unsigned get_channel_count() const
    {
        return get_channels();
    }
    ...
    inline unsigned get_sample_rate() const
    {
        return get_srate();
    }
    //! Helper, same as set_srate().
    inline void set_sample_rate(unsigned val)
    {
        set_srate(val);
    }
    ...
    inline void grow_data_size(t_size p_requested)
    {
    ...
    {
        return get_sample_count() * get_channels();
    }
...
#pragma deprecated( get_data_length )
    ...
    inline void set_data(const audio_sample * src,t_size samples,unsigned nch,unsigned srate)
    {
        set_data(src,samples,nch,srate,g_guess_channel_config(nch));
    }
    ...
    inline void set_data_32(const float * src,t_size samples,unsigned nch,unsigned srate)
    {
...
public:
    ...
    audio_chunk_impl_t(const audio_chunk & p_source) : m_srate(0), m_nch(0), m_samples(0), m_setup(0)
    {
        copy(p_source);
    }
    audio_chunk_impl_t(const t_self & p_source) : m_srate(0), m_nch(0), m_samples(0), m_setup(0)
    {
        copy(p_source);
    }
...
public:
    ...
    template<typename t_source> duration_counter & operator+=(const t_source & source)
    {
        add(source);
        return *this;
    }
