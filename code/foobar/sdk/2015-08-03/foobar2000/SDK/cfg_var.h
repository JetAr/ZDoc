...
public:
    ...
    cfg_var_reader(const GUID & guid) : m_guid(guid)
    {
...
public:
    //! @param p_guid GUID of the variable, used to identify variable implementations owning specific configuration file entries when reading the configuration file back. You must generate a new GUID every time you declare a new cfg_var.
    cfg_var_writer(const GUID & guid) : m_guid(guid)
    {
        CFG_VAR_ASSERT_SAFEINIT;
        m_next = g_list;
        g_list = this;
...
protected:
    ...
    {
        p_stream->write_lendian_t(m_val,p_abort);
    }
...
public:
    ...
    inline const cfg_int_t<t_inttype> & operator=(const cfg_int_t<t_inttype> & p_val)
    {
        m_val=p_val.m_val;
        return *this;
    }
    inline t_inttype operator=(t_inttype p_val)
    {
        m_val=p_val;
        return m_val;
    }

    inline operator t_inttype() const
    {
        return m_val;
    }

    inline t_inttype get_value() const
    {
        return m_val;
    }
};
...
public:
    ...
    inline const cfg_string& operator=(const cfg_string & p_val)
    {
        set_string(p_val);
        return *this;
    }
    inline const cfg_string& operator=(const char* p_val)
    {
        set_string(p_val);
        return *this;
    }
...
protected:
    ...
    void get_data_raw(stream_writer * p_stream,abort_callback & p_abort)
    {
        p_stream->write_object(&m_val,sizeof(m_val),p_abort);
    }
    void set_data_raw(stream_reader * p_stream,t_size p_sizehint,abort_callback & p_abort)
    {
        t_struct temp;
...
public:
    ...
    //! @param p_guid GUID of the variable, used to identify variable implementations owning specific configuration file entries when reading the configuration file back. You must generate a new GUID every time you declare a new cfg_var.
    inline cfg_struct_t(const GUID & p_guid,int filler) : cfg_var(p_guid)
    {
        memset(&m_val,filler,sizeof(t_struct));
    }

    inline const cfg_struct_t<t_struct> & operator=(const cfg_struct_t<t_struct> & p_val)
    {
        m_val = p_val.get_value();
        return *this;
    }
    inline const cfg_struct_t<t_struct> & operator=(const t_struct & p_val)
    {
        m_val = p_val;
        return *this;
    }

    inline const t_struct& get_value() const
    {
        return m_val;
    }
    inline t_struct& get_value()
    {
        return m_val;
    }
    inline operator t_struct() const
    {
        return m_val;
    }
};
...
public:
    ...
    {
        this->set_size(Count);
        for(t_size walk = 0; walk < Count; ++walk) (*this)[walk] = source[walk];
    }
    void get_data_raw(stream_writer * p_stream,abort_callback & p_abort)
    {
...
private:
    ...
    void set_data_raw(stream_reader * p_stream,t_size p_sizehint,abort_callback & p_abort)
    {
        ...
        for(t_uint32 walk = 0; walk < count; ++walk)
        {
            typename TMap::t_key key;
            in >> key;
            PFC_ASSERT( !this->have_item(key) );
            try
            {
                in >> this->find_or_add( key );
            }
            catch(...)
            {
                this->remove(key);
