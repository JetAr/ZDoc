...
{
    get_data_struct_t<bool>(p_out);
}
void config_object::set_data_bool(bool p_val)
{
    set_data_struct_t<bool>(p_val);
}

void config_object::g_get_data_bool(const GUID & p_guid,bool & p_out)
{
    g_get_data_struct_t<bool>(p_guid,p_out);
}
void config_object::g_set_data_bool(const GUID & p_guid,bool p_val)
{
    g_set_data_struct_t<bool>(p_guid,p_val);
}
...
public:
    ...
    stream_writer_string(pfc::string_base & p_out) : m_out(p_out)
    {
        m_out.reset();
    }
private:
...
public:
    ...
    stream_writer_fixedbuffer(void * p_out,t_size p_bytes,t_size & p_bytes_read) : m_out(p_out), m_bytes(p_bytes), m_bytes_read(p_bytes_read)
    {
        m_bytes_read = 0;
    }
private:
...
public:
    ...
    stream_writer_get_length(t_size & p_length) : m_length(p_length)
    {
        m_length = 0;
    }
private:
