...
public:
    ...
    {
        return insert_item(get_count(),hint_size);
    }
...
public:
    ...
    {
        set_owner(p_source.get_owner());
        set_data(p_source.get_data(),p_source.get_data_size());
    }
...
public:
    ...
    dsp_preset_reader(const dsp_preset_reader & p_source) : m_walk(0)
    {
        *this = p_source;
    }
...
public:
    ...
    {
        m_data.set_data_fromptr((const t_uint8*)p_data,p_data_size);
    }
...
public:
    ...
    bool get_default_preset(dsp_preset & p_out)
    {
        return T::g_get_default_preset(p_out);
    }
    bool instantiate(service_ptr_t<dsp> & p_out,const dsp_preset & p_preset)
    {
    ...
    bool have_config_popup()
    {
        return T::g_have_config_popup();
    }
    bool show_config_popup(dsp_preset & p_data,HWND p_parent)
    {
        return T::g_show_config_popup(p_data,p_parent);
    }
...
{
    value.contents_from_stream(&stream.m_stream, stream.m_abort);
    return stream;
}

FB2K_STREAM_WRITER_OVERLOAD(dsp_chain_config)
{
    value.contents_to_stream(&stream.m_stream, stream.m_abort);
    return stream;
}
...
public:
    ...
    const dsp_preset & get_item(t_size p_index) const
    {
        return m_data.get_item(p_index);
    }
