...
void archive_impl::make_unpack_path(pfc::string_base & path,const char * archive,const char * file)
{
    g_make_unpack_path(path,archive,file,get_archive_type());
}
...
public:
    ...
    exception_io_win32_ex(const exception_io_win32_ex & p_other)
    {
        *this = p_other;
    }
...
void stream_reader::read_string_nullterm( pfc::string_base & out, abort_callback & abort )
{
    ...
    for(;;)
    {
        ...
        if (c == 0)
        {
            out.add_string( buffer, w );
            break;
        }
        if (++w == bufCount )
        {
            out.add_string( buffer, bufCount );
            w = 0;
        }
    }
}
