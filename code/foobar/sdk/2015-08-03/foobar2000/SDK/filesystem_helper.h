...
public:
    ...
    {
        filesystem::g_get_canonical_path(src,m_data);
    }
...
public:
    ...
    {
        filesystem::g_get_display_path(src,m_data);
    }
    operator const char * () const
    {
        return m_data.get_ptr();
...
protected:
    ...
    inline void seek_internal(t_size p_offset)
    {
        if (p_offset > get_buffer_size()) throw exception_io_seek_out_of_range();
        m_offset = p_offset;
    }
private:
    t_size m_offset;
...
public:
    ...
    t_filetimestamp get_timestamp(abort_callback & p_abort)
    {
        return r->get_timestamp(p_abort);
    }
...
public:
    ...
    inline t_filesize get_remaining() const
    {
        return m_remaining;
    }
...
class stream_reader_dummy : public stream_reader
{
    t_size read(void * p_buffer,t_size p_bytes,abort_callback & p_abort)
    {
        return 0;
...
public:
    ...
    {
        m_stream.skip_object(p_bytes,m_abort);
    }
...
public:
    ...
    {
        _m_stream.set_data(source,sourceSize);
    }
    template<typename TSource> void set_data(const TSource & source)
    {
        _m_stream.set_data(source);
    }
    ...
    {
        return _m_stream.get_remaining();
    }
...
public:
    ...
    {
        set_data(source,sourceSize);
    }
    template<typename TSource> stream_reader_formatter_simple(const TSource & source)
    {
        set_data(source);
    }
...
public:
    ...
    template<typename t_what> t_self & operator||(t_what & out)
    {
        m_stream >> out;
        return *this;
    }
...
public:
    ...
    template<typename t_what> t_self & operator||(const t_what & in)
    {
        m_stream << in;
        return *this;
    }
...
public:
    ...
    {
        return new service_impl_t< file_chain_readonly > ( chain );
    }
};
