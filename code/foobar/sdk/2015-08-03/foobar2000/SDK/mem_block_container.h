...
public:
    ...
    inline void copy(const mem_block_container & p_source)
    {
        set(p_source.get_ptr(),p_source.get_size());
    }
    inline void reset()
    {
        set_size(0);
...
public:
    ...
    void set_size(t_size p_size)
    {
        if (p_size > m_buffer_size) throw pfc::exception_overflow();
        m_size = p_size;
    }
private:
...
public:
    ...
    t_size get_size() const
    {
        return m_ref.get_size();
    }
    void set_size(t_size p_size)
    {
        m_ref.set_size(p_size);
    }
private:
    t_ref & m_ref;
};
