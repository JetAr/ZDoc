...
public:
    ...
    {
        const t_size count = source.get_entry_count();
        set_entry_count(count);
        metadb_handle_list content;
        pfc::string8 name;
        for(t_size walk = 0; walk < count; ++walk)
        {
            source.get_entry_name(walk,name);
            source.get_entry_content(walk,content);
            set_entry_name(walk,name);
            set_entry_content(walk,content);
        }
    }
protected:
...
public:
    ...
    void copy(playlist_dataobject_desc_v2 const & source)
    {
        ...
        for(t_size walk = 0; walk < count; ++walk)
        {
            source.get_entry_name(walk,name);
            source.get_entry_content(walk,content);
            source.get_side_data(walk, sideData);
            set_entry_name(walk,name);
            set_entry_content(walk,content);
            set_side_data(walk, sideData.get_ptr(), sideData.get_size());
        }
    }
        ...
        for(t_size walk = 0; walk < pltotal; ++walk) if (mask[walk])
            {
                ...
                api->playlist_get_name(walk,name);
                api->playlist_get_all_items(walk,content);
                set_entry_name(done,name);
                set_entry_content(done,content);
                stream_writer_buffer_simple sideData;
                api->playlist_get_sideinfo(walk, &sideData, abort);
