...
struct t_pcmspec
{
    ...
    inline t_pcmspec(const t_pcmspec & p_source)
    {
        *this = p_source;
    }
    ...
    {
        return (t_size)audio_math::time_to_samples(p_time,m_sample_rate) * (m_bits_per_sample / 8) * m_channels;
    }
    double bytes_to_time(t_size p_bytes) const
    {
        return (double) (p_bytes / ((m_bits_per_sample / 8) * m_channels)) / (double) m_sample_rate;
    }
...
struct t_samplespec
{
    ...
    inline t_samplespec(audio_chunk const & in)
    {
        fromchunk(in);
    }
...
public:
    ...
    {
        T::g_advanced_settings_popup(p_parent,p_menupoint);
    }
