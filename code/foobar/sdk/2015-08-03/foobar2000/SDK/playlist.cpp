...
public:
    ...
    inline const metadb_handle_ptr & get_item()
    {
        return m_item;
    }
};
...
public:
    enum_items_callback_retrieve_all_items(pfc::list_base_t<metadb_handle_ptr> & p_out) : m_out(p_out)
    {
        m_out.remove_all();
    }
    bool on_item(t_size p_index,const metadb_handle_ptr & p_location,bool b_selected)
...
bool playlist_manager::playlist_move_selection(t_size p_playlist,int p_delta)
{
    ...
    pfc::array_t<bool> selection;
    selection.set_size(count);
...
{
    pfc::com_ptr_t<interface IDataObject> temp;
    temp.attach( create_dataobject(data) );
    PFC_ASSERT( temp.is_valid() );
    return temp;
}
