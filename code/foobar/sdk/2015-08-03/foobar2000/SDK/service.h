...
template<typename T> static inline service_obscure_refcounting<T>* service_obscure_refcounting_cast(T * p_source) throw()
{
    return static_cast<service_obscure_refcounting<T>*>(p_source);
}
...
public:
    ...
    {
        _init(p_source.get_ptr());
    }
    service_ptr_t(t_self && p_source) throw()
    {
        _init(std::move(p_source));
    }
    template<typename t_source> service_ptr_t(t_source * p_ptr) throw()
    {
        _init(p_ptr);
    }
    template<typename t_source> service_ptr_t(const service_ptr_base_t<t_source> & p_source) throw()
    {
        _init(p_source.get_ptr());
    }
    template<typename t_source> service_ptr_t(const service_nnptr_t<t_source> & p_source) throw()
    {
        this->m_ptr = p_source.get_ptr();
        this->m_ptr->service_add_ref();
    }
    template<typename t_source> service_ptr_t(service_ptr_t<t_source> && p_source) throw()
    {
        _init(std::move(p_source));
    }
    ...
    inline const t_self & operator=(t_self && p_source) throw()
    {
        copy(std::move(p_source));
        return *this;
    }
    inline const t_self & operator=(T * p_ptr) throw()
    {
        copy(p_ptr);
        return *this;
    }

    template<typename t_source> inline t_self & operator=(const service_ptr_base_t<t_source> & p_source) throw()
    {
        copy(p_source);
        return *this;
    }
    template<typename t_source> inline t_self & operator=(service_ptr_t<t_source> && p_source) throw()
    {
        copy(std::move(p_source));
        return *this;
    }
    template<typename t_source> inline t_self & operator=(t_source * p_ptr) throw()
    {
        copy(p_ptr);
        return *this;
    }
    ...
    inline service_obscure_refcounting<T>* operator->() const throw()
    {
        PFC_ASSERT(this->m_ptr != NULL);
        return service_obscure_refcounting_cast(this->m_ptr);
    }
    ...
    inline bool operator==(const service_ptr_base_t<T> & p_item) const throw()
    {
    ...
    inline bool operator==(T * p_item) const throw()
    {
    ...
    inline t_self & operator<<(service_ptr_t<t_other> & p_source) throw()
    {
        attach(p_source.detach());
        return *this;
    }
    template<typename t_other>
    inline t_self & operator>>(service_ptr_t<t_other> & p_dest) throw()
    {
...
public:
    ...
    template<typename t_source> service_nnptr_t(t_source * p_ptr) throw()
    {
        _init(p_ptr);
    }
    template<typename t_source> service_nnptr_t(const service_ptr_base_t<t_source> & p_source) throw()
    {
        _init(p_source.get_ptr());
    }

    template<typename t_source> service_nnptr_t(service_ptr_t<t_source> && p_source) throw()
    {
        this->m_ptr = p_source.detach();
    }
...
{
protected:
    inline service_factory_base(const GUID & p_guid, service_factory_base * & factoryList) : m_guid(p_guid)
    {
        PFC_ASSERT(!core_api::are_services_available());
        __internal__next=factoryList;
        factoryList=this;
    }
    inline ~service_factory_base()
    {
        PFC_ASSERT(!core_api::are_services_available());
    }
public:
    inline const GUID & get_class_guid() const
    {
        return m_guid;
    }
    ...
    inline static bool is_service_present(const GUID & g)
    {
        return enum_get_count(enum_find_class(g))>0;
    }
...
public:
    ...
    {
        return service_obscure_refcounting_cast(m_ptr);
    }
    ...
    {
        return *this;   //obsolete, each instance should carry the same pointer
    }
private:
    t_interface * m_ptr;
};
...
public:
    ...
    inline T& get_static_instance()
    {
        return g_instance;
    }
    inline const T& get_static_instance() const
    {
        return g_instance;
    }
};
...
public:
    ...
    {
        service_ptr_t<what> temp;
        if (!crete(temp,theID)) throw exception_service_not_found();
        return temp;
    }
...
{
public:
    template<typename what> static int compare(const what & v1, const what & v2)
    {
