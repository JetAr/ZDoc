...
public:
    ...
    {
        return test_flags(flag_removable | flag_linearlist);
    }
    bool can_reorder()
    {
        return test_flags(flag_reorderable);
    }
    bool can_search()
    {
        return test_flags(flag_searchable);
    }

    virtual void select_all()
    {
        update_selection(bit_array_true(), bit_array_true());
    }
    virtual void select_none()
    {
        update_selection(bit_array_true(), bit_array_false());
    }
    virtual void get_selected_items(metadb_handle_list_ref out)
    {
        bit_array_bittable mask(get_item_count());
        get_selection_mask(mask);
        get_items(out, mask);
    }
    ...
    virtual void crop_selection()
    {
        bit_array_bittable mask(get_item_count());
        get_selection_mask(mask);
        remove_items(bit_array_not(mask));
    }
    virtual void clear()
    {
    ...
    virtual void get_selection_mask(pfc::bit_array_var & out)
    {
        ...
        for(t_size walk = 0; walk < count; ++walk) out.set(walk, is_item_selected(walk));
    }
    ...
    void sort_by_format(const char * spec, bool onlySelection)
    {
...

        {
            ...
            {
                static_api_ptr_t<genrand_service> api;
                api->seed((unsigned)__rdtsc());
