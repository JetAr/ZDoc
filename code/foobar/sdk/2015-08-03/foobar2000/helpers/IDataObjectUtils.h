...
public:
    ...
    ~ReleaseStgMediumScope()
    {
        if (m_medium != NULL) ReleaseStgMedium(m_medium);
    }
private:
    STGMEDIUM * m_medium;
...
public:
    ...
    {
        int val;
        val = pfc::compare_t(v1.cfFormat,v2.cfFormat);
        if (val != 0) return val;
        val = pfc::compare_t(v1.dwAspect,v2.dwAspect);
        if (val != 0) return val;
        val = pfc::compare_t(v1.lindex,  v2.lindex  );
        if (val != 0) return val;
        return 0;
    }
};
