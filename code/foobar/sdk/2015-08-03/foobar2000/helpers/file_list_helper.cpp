...
static void file_list_remove_duplicates(pfc::ptr_list_t<char> & out)
{
    ...
    for(n=1; n<m; n++)
    {
        if (!metadb::path_compare(out[n-1],out[n]))
        {
            duplicates++;
            mask.set(n,true);
        }
...
t_size file_list_from_metadb_handle_list::get_count() const
{
    return m_data.get_count();
}
void file_list_from_metadb_handle_list::get_item_ex(const char * & p_out,t_size n) const
{
    p_out = m_data.get_item(n);
}
