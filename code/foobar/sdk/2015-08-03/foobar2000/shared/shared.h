...
// New in 1.1.12
    ...
    {
    public:
        inline t_size GetCount()
        {
            return get_count();
        }
        inline const char * GetFileName(t_size index)
        {
...
// new in fb2k 1.1.1
    ...
    public:
        ...
        {
            return (GetAttributes() & FILE_ATTRIBUTE_DIRECTORY) ? true : false;
        }
    };
...
static inline void uAddDebugEvent(const char * msg)
{
    uPrintCrashInfo_OnEvent(msg, strlen(msg));
}
static inline void uAddDebugEvent(pfc::string_formatter const & msg)
{
    uPrintCrashInfo_OnEvent(msg, msg.length());
}

inline char * uCharNext(char * src)
{
    return src+uCharLength(src);
}
inline const char * uCharNext(const char * src)
{
    return src+uCharLength(src);
}
...
inline LRESULT uButton_SetCheck(HWND wnd,UINT id,bool state)
{
    return uSendDlgItemMessage(wnd,id,BM_SETCHECK,state ? BST_CHECKED : BST_UNCHECKED,0);
}
inline bool uButton_GetCheck(HWND wnd,UINT id)
{
    return uSendDlgItemMessage(wnd,id,BM_GETCHECK,0,0) == BST_CHECKED;
}
...
{
public:
    explicit string_lower(const char * ptr,t_size p_count = ~0)
    {
        uAddStringLower(m_data,ptr,p_count);
    }
    inline operator const char * () const
    {
        return m_data.get_ptr();
...
{
public:
    explicit string_upper(const char * ptr,t_size p_count = ~0)
    {
        uAddStringUpper(m_data,ptr,p_count);
    }
    inline operator const char * () const
    {
        return m_data.get_ptr();
...
public:
    ...
    inline modal_dialog_scope(HWND p_wnd) throw() : m_initialized(false)
    {
        initialize(p_wnd);
    }
    ...
    inline ~modal_dialog_scope() throw()
    {
        deinitialize();
    }
    ...
    inline static bool can_create() throw()
    {
        return ModalDialog_CanCreateNew();
    }
    //! Activates the top-level modal dialog existing, if one exists.
    inline static void poke_existing() throw()
    {
