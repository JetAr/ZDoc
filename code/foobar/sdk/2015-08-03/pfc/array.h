...
public:
    ...
    template<typename t_source> bool is_owned(const t_source & p_item)
    {
        return pfc::is_pointer_in_range(get_ptr(),get_size(),&p_item);
    }

    template<typename t_out> void enumerate(t_out & out) const
    {
        for(t_size walk = 0; walk < m_size; ++walk) out(m_array[walk]);
    }
private:
    void release_()
    {
...
public:
    ...
    template<typename t_source> array_t(const t_source & p_source)
    {
        copy_array_t(*this,p_source);
    }
    const t_self & operator=(const t_self & p_source)
    ...
    template<typename t_source> const t_self & operator=(const t_source & p_source)
    {
        copy_array_t(*this,p_source);
        return *this;
    }
    ...
    template<typename t_append> void append_multi(const t_append & value, t_size count)
    {
...
public:
    ...
    template<typename t_filler> void fill(const t_filler & p_filler)
    {
        m_content.fill(p_filler);
    }
    void fill_null()
    {
        m_content.fill_null();
