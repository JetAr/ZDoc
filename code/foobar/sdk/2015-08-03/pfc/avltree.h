...
public:
    ...
    inline t_rawptr child(bool which) const throw()
    {
        return which ? m_right.get_ptr() : m_left.get_ptr();
    }
    inline bool which_child(const t_self* ptr) const throw()
    {
        return ptr == m_right.get_ptr();
    }
...
public:
    ...
    avltree_t(const t_self & p_other) : m_root(NULL)
    {
        try
        {
            __copy(p_other);
    ...
    template<typename t_other> const t_self & operator=(const t_other & p_other)
    {
        copy_list_enumerated(*this,p_other);
        return *this;
    }
    template<typename t_other> avltree_t(const t_other & p_other) : m_root(NULL)
    {
        try
        {
            copy_list_enumerated(*this,p_other);
    ...
    template<typename t_param> const_iterator find(t_param const & item) const
    {
        return _find_node_ptr(item);
    }

    //! Unsafe! Caller must not modify items in a way that changes sort order!
    template<typename t_param> iterator find(t_param const & item)
    {
        return _find_node_ptr(item);
    }
    ...
    {
        __enum_items_recur<t_node>(m_root.get_ptr(),p_callback);
    }
    ...
    template<typename t_param> t_storage & add(const t_param & p_item)
    {
        return add_item(p_item);
    }
    template<typename t_param> t_storage & add_ex(const t_param & p_item,bool & p_isnew)
    {
        return add_item_ex(p_item,p_isnew);
    }
    template<typename t_param> const t_storage * find_ptr(t_param const & p_item) const
    {
        return find_item_ptr(p_item);
    }
    template<typename t_param> t_storage * find_ptr(t_param const & p_item)
    {
        return find_item_ptr(p_item);
    }
    template<typename t_param> bool exists(t_param const & p_item) const
    {
    ...
    {
        return _firstlast(false);
    }
    const_iterator last() const throw()
    {
        return _firstlast(true);
    }
    //! Unsafe! Caller must not modify items in a way that changes sort order!
    iterator _first_var()
    {
