...
public:
    ...
    inline T operator[](t_size n) const
    {
        T temp;
        get_item_ex(temp,n);
        return std::move(temp);
    ...
    template<typename t_search>
    inline bool have_item(const t_search & p_item) const
    {
...
private:
    ...
    void get_item_ex(T & p_out,t_size n) const
    {
        m_list.get_item_ex(p_out,n+m_base);
    }
};
...
public:
    ...
    inline void reorder(const t_size * p_data)
    {
        reorder_partial(0,p_data,this->get_count());
    }

    inline t_size insert_item(const T & item,t_size base)
    {
        return insert_items(list_single_ref_t<T>(item),base);
    }
    t_size insert_items_repeat(const T & item,t_size num,t_size base)
    {
        return insert_items(list_single_ref_t<T>(item,num),base);
    }
    inline t_size add_items_repeat(T item,t_size num)
    {
        return insert_items_repeat(item,num,~0);
    }
    t_size insert_items_fromptr(const T* source,t_size num,t_size base)
    {
        return insert_items(list_const_ptr_t<T>(source,num),base);
    }
    inline t_size add_items_fromptr(const T* source,t_size num)
    {
        return insert_items_fromptr(source,num,~0);
    }

    inline t_size add_items(const list_base_const_t<T> & items)
    {
        return insert_items(items,~0);
    }
    inline t_size add_item(const T& item)
    {
        return insert_item(item,~0);
    }

    inline void remove_mask(const bit_array & mask)
    {
        filter_mask(bit_array_not(mask));
    }
    inline void remove_all()
    {
        filter_mask(bit_array_false());
    }
    inline void truncate(t_size val)
    {
        if (val < this->get_count()) remove_mask(bit_array_range(val,this->get_count()-val,true));
    }

    inline T replace_item_ex(t_size p_index,const T & p_item)
    {
        T ret = p_item;
        swap_item_with(p_index,ret);
        return ret;
    }
    ...
    void sort()
    {
        sort(sort_callback_auto());
    }
    template<typename t_compare> void sort_t(t_compare p_compare)
    {
        sort(sort_callback_impl_t<t_compare>(p_compare));
    }
    template<typename t_compare> void sort_stable_t(t_compare p_compare)
    {
        sort_stable(sort_callback_impl_t<t_compare>(p_compare));
    }
    ...
    template<typename t_in> t_self & operator+=(t_in const & p_source)
    {
        add_item(p_source);
        return *this;
    }
...
public:
    ...
    {
        m_buffer.prealloc(count);
    }

    void set_count(t_size p_count)
    {
        m_buffer.set_size(p_count);
    }
    ...
    {
        return _insert_items_v(source, base);
    }
...
private:
    ...
    public:
        ...
        inline int operator()(const T& item1,const T& item2) const
        {
...
public:
    ...
    {
        remove_mask(bit_array_table(mask,get_size()));
    }
    void filter_mask(const bool * mask)
    {
        filter_mask(bit_array_table(mask,get_size()));
    }
    ...
    inline bool have_item(const t_search & p_item) const
    {
        return this->template find_item<t_search>(p_item)!=~0;
    }
...
public:
    ...
    template<typename t_in> t_self & operator+=(t_in const & p_source)
    {
        this->add_item(p_source);
        return *this;
    }
    template<typename t_in> t_self & operator|=(t_in const & p_source)
    {
        this->add_items(p_source);
        return *this;
    }
};
...
public:
    ...
    t_size get_count() const
    {
        return m_param.get_count();
    }
    void get_item_ex(const T * & p_out,t_size n) const
    {
        T* temp;
        m_param.get_item_ex(temp,n);
        p_out = temp;
    }
private:
...
public:
    ...
    void get_item_ex(T & p_out,t_size n) const
    {
        m_list.get_item_ex(p_out,m_permutation[n]);
    }
private:
    P m_permutation;
    const list_base_const_t<T> & m_list;
...
public:
    ...
    void get_item_ex(T & p_out,t_size n) const
    {
        m_base.get_item_ex(p_out,m_order[n]);
    }
