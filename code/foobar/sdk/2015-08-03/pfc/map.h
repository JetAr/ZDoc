...
public:
    ...
    template<typename t_key,typename t_value> void operator() (const t_key & p_key,const t_value & p_value)
    {
...
public:
    ...
    template<typename _t_key> bool exists(const _t_key & p_key) const
    {
...
private:
    ...
    struct t_retrieve_key
    {
        ...
        template<typename t_what> const t_self & operator=(const t_what & p_what)
        {
            m_key = p_what.m_key;
            return *this;
        }
    ...
    struct t_storage
    {
        ...
        {
            return v1.m_key == v2.m_key && v1.m_value == v2.m_value;
        }
    ...
    public:
        ...
        {
            m_callback(p_item.m_key,p_item.m_value);
        }
    private:
    ...
    public:
        ...
        {
            m_callback(implicit_cast<t_storage_key const&>(p_item.m_key),p_item.m_value);
        }
    private:
        t_callback & m_callback;
    };
...
public:
    ...
    {
        return m_data._first_var();
    }
    iterator last() throw()
    {
        return m_data._last_var();
    }
    ...
    template<typename _t_key> iterator find(const _t_key & key)
    {
        return m_data.find(t_search_query<_t_key>(key));
    }
    template<typename _t_key> const_iterator find(const _t_key & key) const
    {
        return m_data.find(t_search_query<_t_key>(key));
    }
