...
{
public:
    template<typename T> static void release(T* p_ptr)
    {
        delete p_ptr;
    }
};
...
public:
    ...
    inline void release()
    {
        t_releaser::release(replace_null_t(m_ptr));;
    }
    inline void attach(T * p_ptr)
    {
        release();
        m_ptr = p_ptr;
    }
    inline const t_self & operator=(T * p_ptr)
    {
        set(p_ptr);
        return *this;
    }
    ...
    inline t_self & operator<<(t_self & p_source)
    {
        attach(p_source.detach());
        return *this;
    }
    inline t_self & operator>>(t_self & p_dest)
    {
...
public:
    ...
    instanceTracker(const t_self & p_other) : t_base( (const t_base &)p_other)
    {
        g_list += this;
    }
    ~instanceTracker()
    ...
    template<typename t_callback> static void forEach(t_callback & p_callback)
    {
        instanceList().enumerate(p_callback);
    }
private:
...
public:
    ...
    {
        g_list += static_cast<TClass*>(this);
    }
    instanceTrackerV2()
    {
        g_list += static_cast<TClass*>(this);
