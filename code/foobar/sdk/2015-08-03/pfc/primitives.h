...
public:
    ...
    __exception_with_message_t(const t_self & p_source) : m_message(NULL)
    {
    ...
    {
        return m_message != NULL ? m_message : "unnamed exception";
    }
...
{
    if (traits_t<t_type>::needs_destructor) try
        {
            p_item.~t_type();
        }
        catch(...) {}
...
template<typename t_ret> t_ret implicit_cast(t_ret val)
{
    return val;
}
...
template<typename t_comparator = pfc::comparator_default> class comparator_pointer
{
public:
...
template<typename t_primary,typename t_secondary> class comparator_dual
{
public:
    template<typename t_item1,typename t_item2> static int compare(const t_item1 & p_item1,const t_item2 & p_item2)
    {
...
template<typename T>
inline void delete_array_t(T* ptr)
{
    delete[] ptr;
}

template<typename T>
inline T* clone_t(T* ptr)
{
    return new T(*ptr);
}
...
template<> inline const char * empty_string_t<char>()
{
    return "";
}
template<> inline const wchar_t * empty_string_t<wchar_t>()
{
    return L"";
}
...
void array_rangecheck_t(const t_array & p_array,t_size p_from,t_size p_to)
{
    ...
    array_rangecheck_t(p_array,p_from);
    array_rangecheck_t(p_array,p_to);
}
...
public:
    ...
    template<typename t_item> void operator() (const t_item & p_item)
    {
        m_receiver.add_item(p_item);
    }
private:
