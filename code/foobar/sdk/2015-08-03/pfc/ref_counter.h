...
public:
    ...
    {
        --m_counter;   //for internal use only!
    }
protected:
...
public:
    ...
    inline refcounted_object_ptr_t(T* p_ptr) throw() : m_ptr(NULL)
    {
        copy(p_ptr);
    }
    inline refcounted_object_ptr_t(const t_self & p_source) throw() : m_ptr(NULL)
    {
        copy(p_source);
    }
    inline refcounted_object_ptr_t(t_self && p_source) throw()
    {
    ...
    inline refcounted_object_ptr_t(t_source * p_ptr) throw() : m_ptr(NULL)
    {
        copy(p_ptr);
    }

    template<typename t_source>
    inline refcounted_object_ptr_t(const refcounted_object_ptr_t<t_source> & p_source) throw() : m_ptr(NULL)
    {
        copy(p_source);
    }
    ...
    {
        if (m_ptr != NULL) m_ptr->refcount_release();
    }
    ...
    template<typename t_source>
    inline void copy(const refcounted_object_ptr_t<t_source> & p_source) throw()
    {
    ...
    template<typename t_source> inline t_self & operator=(const refcounted_object_ptr_t<t_source> & p_source) throw()
    {
        copy(p_source);
        return *this;
    }
    ...
    inline bool operator==(const t_self & p_item) const throw()
    {
