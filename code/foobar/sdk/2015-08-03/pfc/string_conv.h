...
template<> inline const char * null_string_t<char>()
{
    return "";
}
template<> inline const wchar_t * null_string_t<wchar_t>()
{
    return L"";
}
...
public:
    ...
    bool is_empty() const
    {
        return string_is_empty_t(get_ptr());
    }
    t_size length() const
    {
        return strlen_t(get_ptr());
    }
...
public:
    ...
    void convert(const char * p_source,t_size p_source_size = ~0)
    {
        m_buffer.convert(codepage_system,p_source,p_source_size);
    }
...
inline pfc::string_base & operator<<(pfc::string_base & p_fmt, const wchar_t * p_str)
{
    p_fmt.add_string(pfc::stringcvt::string_utf8_from_wide(p_str) );
    return p_fmt;
}
