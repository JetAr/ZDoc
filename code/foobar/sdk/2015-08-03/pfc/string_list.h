...
public:
    ...
    {
        pfc::append_t(m_data, p_string);
    }

    template<typename t_what> void add_items(const t_what & p_source)
    {
        _append(p_source);
    }
    ...
    string_list_impl() {}
    template<typename t_what> string_list_impl(const t_what & p_source)
    {
        _copy(p_source);
    }
    template<typename t_what> string_list_impl & operator=(const t_what & p_source)
    {
        _copy(p_source);
        return *this;
    }
    template<typename t_what> string_list_impl & operator|=(const string_list_impl & p_source)
    {
        _append(p_source);
        return *this;
    }
