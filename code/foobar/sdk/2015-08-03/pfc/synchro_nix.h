...
public:
    ...
    {
        int rv = 0;
        pthread_mutexattr_gettype(&attr, &rv);
        return rv;
    }
    void setRecursive()
    {
    ...
    bool isRecursive()
    {
        return getType() == PTHREAD_MUTEX_RECURSIVE;
    }
...
{
public:
    mutexScope( mutexBase * m ) throw() : m_mutex(m)
    {
        m_mutex->enter();
    }
    mutexScope( mutexBase & m ) throw() : m_mutex(&m)
...
{
public:
    _readWriteLock_scope_read( readWriteLockBase & lock ) : m_lock( lock )
    {
        m_lock.enterRead();
    }
...
{
public:
    _readWriteLock_scope_write( readWriteLockBase & lock ) : m_lock( lock )
    {
        m_lock.enterWrite();
    }
