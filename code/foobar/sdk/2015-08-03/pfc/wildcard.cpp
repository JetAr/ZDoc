...
static bool test_recur(const char * fn,const char * rm,bool b_sep)
{
    for(;;)
    {
        ...
        fn = pfc::utf8_char_next(fn);
        rm = pfc::utf8_char_next(rm);
    }
}

bool wildcard_helper::test_path(const char * path,const char * pattern,bool b_sep)
{
    return test(path + pfc::scan_filename(path),pattern,b_sep);
}
