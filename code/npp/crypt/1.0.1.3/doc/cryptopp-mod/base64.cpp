...
void Base64Encoder::IsolatedInitialize(const NameValuePairs &parameters)
{
    ...
    EOL eol = parameters.GetValueWithDefault("EOL", EOL::Unix);
    const char *slineBreaks[] = { "", "\n", "\r\n" };
    const char *lineBreak = slineBreaks[0];
    if (insertLineBreaks)
    {
        switch (eol)
        {
        case EOL::Unix:
            lineBreak = slineBreaks[1];
            break;
        case EOL::Windows:
            lineBreak =slineBreaks[2];
            break;
        }
    }
