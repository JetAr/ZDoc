...
struct Crypt
{
    ...
    {
        key.salt_bytes = 16;
        key.algorithm = KeyDerivation::scrypt;
        key.option1 = Constants::scrypt_N_default;
        key.option2 = Constants::scrypt_r_default;
        key.option3 = Constants::scrypt_p_default;
        hmac.enable = false;
        hmac.hash = crypt::Hash::tiger;
        hmac.key_id = 0;
        encoding.enc = crypt::Encoding::base64;
        encoding.linebreaks = true;
        encoding.linelength = 64;
        encoding.uppercase = true;
        encoding.windows = true;
    };
    ...
    struct HMAC
    {
        HMAC& operator= (const HMAC &src)
        {
            enable = src.enable;
            hash = src.hash;
            key_input.assign(src.key_input);
            key_id = src.key_id;
            key.resize(src.key.size());
